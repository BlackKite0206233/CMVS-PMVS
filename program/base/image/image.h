...
Vec3f Cimage::getColor(const float x, const float y, const int level) const {
  ...
  */
  ...
  return Vec3f(m_images[level][index] * f00 + m_images[level][index + 3] * f10 +
               m_images[level][index2] * f01 + m_images[level][index2 + 3] *
  f11,
               

               m_images[level][index + 1] * f00 + m_images[level][index + 4] *
  f10 + m_images[level][index2 + 1] * f01 + m_images[level][index2 + 4] * f11,
               

               m_images[level][index + 2] * f00 + m_images[level][index + 5] *
  f10 + m_images[level][index2 + 2] * f01 + m_images[level][index2 + 5] * f11);
  */
#endif
  /*
