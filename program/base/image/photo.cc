...
float Cphoto::ssd(const std::vector<Vec3f> &tex0, const std::vector<Vec3f> &tex1) {
  float ans = 0.0f;
...
float Cphoto::idot(const std::vector<Vec3f> &tex0, const std::vector<Vec3f> &tex1) {
...
void Cphoto::idotC(const std::vector<Vec3f> &tex0, const std::vector<Vec3f> &tex1, double *idc) {
  if (tex0.empty() || tex1.empty()) {
...
void Cphoto::grabTex(const int level, const Vec2f &icoord, const Vec2f &xaxis, const Vec2f &yaxis, const int size, std::vector<Vec3f> &tex, const int normalizef) const {
  ...
  tex.clear();
  if (minx < 0 || getWidth(level) - 1 <= maxx || miny < 0 || getHeight(level) - 1 <= maxy)
    return;
