...
public:
  ...
  inline TVec2<T> col(int i) const {
    return TVec2<T>(m_row[0][i], m_row[1][i]);
  }
...
template <class T> inline TMat2<T> &TMat2<T>::operator=(const TMat2<T> &m) {
  m_row[0] = m[0];
  m_row[1] = m[1];
  return *this;
}

template <class T> inline TMat2<T> &TMat2<T>::operator=(T s) {
  m_row[0] = s;
  m_row[1] = s;
  return *this;
}
...
template <class T> inline TMat2<T> operator+(const TMat2<T> &n, const TMat2<T> &m) {
...
template <class T> inline TMat2<T> operator-(const TMat2<T> &m) {
  return TMat2<T>(-m[0], -m[1]);
}

template <class T> inline TMat2<T> operator*(T s, const TMat2<T> &m) {
  return TMat2<T>(m[0] * s, m[1] * s);
}

template <class T> inline TMat2<T> operator*(const TMat2<T> &m, T s) {
  return s * m;
}
...
template <class T> inline TVec2<T> operator*(const TMat2<T> &m, const TVec2<T> &v) {
  return TVec2<T>(m[0] * v, m[1] * v);
}

template <class T> extern TMat2<T> operator*(const TMat2<T> &n, const TMat2<T> &m);

template <class T> inline std::ostream &operator<<(std::ostream &out, const TMat2<T> &M) {
...
template <class T> inline T det(const TMat2<T> &m) {
  return m(0, 0) * m(1, 1) - m(0, 1) * m(1, 0);
}

template <class T> inline T trace(const TMat2<T> &m) {
  return m(0, 0) + m(1, 1);
}

template <class T> inline TMat2<T> transpose(const TMat2<T> &m) {
  return TMat2<T>(m.col(0), m.col(1));
}

template <class T> inline TMat2<T> adjoint(const TMat2<T> &m) {
  return TMat2<T>(perp(m[1]), -perp(m[0]));
}

template <class T> TMat2<T> TMat2<T>::I() { return TMat2<T>(1, 0, 0, 1); }

template <class T> TMat2<T> &TMat2<T>::diag(T d) {
  m_row[0][0] = d;
  m_row[0][1] = 0;
  m_row[1][0] = 0;
  m_row[1][1] = d;
...
template <class T> TMat2<T> operator*(const TMat2<T> &n, const TMat2<T> &m) {
  TMat2<T> A;
  int i, j;
...
template <class T> T invert(TMat2<T> &inv, const TMat2<T> &m) {
  T d = det(m);
...
template <class T> bool eigenvalues(const TMat2<T> &M, TVec2<T> &evals) {
  T B = -M(0, 0) - M(1, 1);
  T C = det(M);
...
template <class T> bool eigenvectors(const TMat2<T> &M, const TVec2<T> &evals, TVec2<T> evecs[2]) {
...
template <class T> bool eigen(const TMat2<T> &M, TVec2<T> &evals, TVec2<T> evecs[2]) {
