...
public:
  ...
  inline TVec3<T> col(int i) const {
    return TVec3<T>(m_row[0][i], m_row[1][i], m_row[2][i]);
  }
...
template <class T> inline TMat3<T> &TMat3<T>::operator=(const TMat3<T> &m) {
  m_row[0] = m[0];
  m_row[1] = m[1];
  m_row[2] = m[2];
  return *this;
...
template <class T> inline TMat3<T> &TMat3<T>::operator=(T s) {
  m_row[0] = s;
  m_row[1] = s;
  m_row[2] = s;
  return *this;
...
template <class T> inline bool TMat3<T>::operator==(const TMat3<T> &m) const {
  return m_row[0] == m.m_row[0] && m_row[1] == m.m_row[1] && m_row[2] == m.m_row[2];
};
...
template <class T> inline TMat3<T> operator+(const TMat3<T> &n, const TMat3<T> &m) {
...
template <class T> inline TMat3<T> operator-(const TMat3<T> &m) {
  return TMat3<T>(-m[0], -m[1], -m[2]);
};

template <class T> inline TMat3<T> operator*(T s, const TMat3<T> &m) {
  return TMat3<T>(m[0] * s, m[1] * s, m[2] * s);
};
template <class T> inline TMat3<T> operator*(const TMat3<T> &m, T s) {
  return s * m;
};
...
template <class T> inline TVec3<T> operator*(const TMat3<T> &m, const TVec3<T> &v) {
  return TVec3<T>(m[0] * v, m[1] * v, m[2] * v);
};

template <class T> inline std::ostream &operator<<(std::ostream &out, const TMat3<T> &M) {
  return out << M[0] << std::endl << M[1] << std::endl << M[2];
};

template <class T> inline std::istream &operator>>(std::istream &in, TMat3<T> &M) {
...
//
template <class T> TMat3<T> TMat3<T>::I() {
  return TMat3<T>(TVec3<T>(1, 0, 0), TVec3<T>(0, 1, 0), TVec3<T>(0, 0, 1));
};

template <class T> TMat3<T> &TMat3<T>::diag(T d) {
  *this = 0.0;
  m_row[0][0] = m_row[1][1] = m_row[2][2] = d;
  return *this;
};

template <class T> TMat3<T> diag(const TVec3<T> &v) {
  return TMat3<T>(TVec3<T>(v[0], 0, 0), TVec3<T>(0, v[1], 0), TVec3<T>(0, 0, v[2]));
};

template <class T> TMat3<T> TMat3<T>::outer_product(const TVec3<T> &v) {
  TMat3<T> A;
  T x = v[0], y = v[1], z = v[2];
...
template <class T> TMat3<T> TMat3<T>::outer_product(const TVec3<T> &u, const TVec3<T> &v) {
  TMat3<T> A;
...
template <class T> TMat3<T> operator*(const TMat3<T> &n, const TMat3<T> &m) {
  TMat3<T> A;
...
template <class T> TMat3<T> adjoint(const TMat3<T> &m) {
  return TMat3<T>(m[1] ^ m[2], m[2] ^ m[0], m[0] ^ m[1]);
};

template <class T> T invert(TMat3<T> &inv, const TMat3<T> &m) {
  TMat3<T> A = adjoint(m);
  T d = A[0] * m[0];
...
template <class T> inline T det(const TMat3<T> &m) {
  return m[0] * (m[1] ^ m[2]);
};

template <class T> inline T trace(const TMat3<T> &m) {
  return m(0, 0) + m(1, 1) + m(2, 2);
};

template <class T> inline TMat3<T> transpose(const TMat3<T> &m) {
  return TMat3<T>(m.col(0), m.col(1), m.col(2));
};

template <class T> inline TMat3<T> row_extend(const TVec3<T> &v) {
...
template <class T> inline TMat3<T> rodrigues(const TVec3<T> &axis) {
  const T theta = norm(axis);
  if (theta == 0)
    return TMat3<T>::I();

  TMat3<T> wx(TVec3<T>(0, -axis[2], axis[1]), TVec3<T>(axis[2], 0, -axis[0]), TVec3<T>(-axis[1], axis[0], 0));
...
template <class T> inline TVec3<T> irodrigues(const TMat3<T> &rot) {
  ...
  TVec3<T> vec(rot[2][1] - rot[1][2], rot[0][2] - rot[2][0], rot[1][0] - rot[0][1]);
