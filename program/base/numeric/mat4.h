...
template <class T> inline TMat4<T> &TMat4<T>::operator=(const TMat4<T> &m) {
  m_row[0] = m[0];
  m_row[1] = m[1];
  m_row[2] = m[2];
  m_row[3] = m[3];
...
template <class T> inline TMat4<T> &TMat4<T>::operator=(T s) {
  m_row[0] = s;
  m_row[1] = s;
  m_row[2] = s;
  m_row[3] = s;
...
template <class T> inline TMat4<T> operator+(const TMat4<T> &n, const TMat4<T> &m) {
...
template <class T> inline TMat4<T> operator-(const TMat4<T> &n) {
  return TMat4<T>(-n[0], -n[1], -n[2], -n[3]);
};

template <class T> inline TMat4<T> operator*(T s, const TMat4<T> &m) {
  return TMat4<T>(m[0] * s, m[1] * s, m[2] * s, m[3] * s);
};
template <class T> inline TMat4<T> operator*(const TMat4<T> &m, T s) {
  return s * m;
};
...
template <class T> inline TVec4<T> operator*(const TMat4<T> &m, const TVec4<T> &v) {
...
//
template <class T> inline TVec3<T> operator*(const TMat4<T> &m, const TVec3<T> &v) {
...
template <class T> inline std::ostream &operator<<(std::ostream &out, const TMat4<T> &M) {
  return out << M[0] << std::endl
             << M[1] << std::endl
             << M[2] << std::endl
             << M[3];
};

template <class T> inline std::istream &operator>>(std::istream &in, TMat4<T> &M) {
...
template <class T> inline TMat4<T> rotation_matrix_deg(T theta, const TVec3<T> &axis) {
...
//
template <class T> inline T det(const TMat4<T> &m) {
  return m[0] * cross(m[1], m[2], m[3]);
}

template <class T> inline T trace(const TMat4<T> &m) {
  return m(0, 0) + m(1, 1) + m(2, 2) + m(3, 3);
}

template <class T> inline TMat4<T> transpose(const TMat4<T> &m) {
  return TMat4<T>(m.col(0), m.col(1), m.col(2), m.col(3));
}

template <class T> TMat4<T> TMat4<T>::I() {
  return TMat4<T>(TVec4<T>(1, 0, 0, 0), TVec4<T>(0, 1, 0, 0), TVec4<T>(0, 0, 1, 0), TVec4<T>(0, 0, 0, 1));
}

template <class T> TMat4<T> translation_matrix(const TVec3<T> &d) {
  return TMat4<T>(TVec4<T>(1, 0, 0, d[0]), TVec4<T>(0, 1, 0, d[1]), TVec4<T>(0, 0, 1, d[2]), TVec4<T>(0, 0, 0, 1));
}

template <class T> TMat4<T> scaling_matrix(const TVec3<T> &s) {
  return TMat4<T>(TVec4<T>(s[0], 0, 0, 0), TVec4<T>(0, s[1], 0, 0), TVec4<T>(0, 0, s[2], 0), TVec4<T>(0, 0, 0, 1));
}

template <class T> TMat4<T> rotation_matrix_rad(T theta, const TVec3<T> &axis) {
  T c  = cos(theta), 
    s  = sin(theta), 
...
template <class T> TMat4<T> perspective_matrix(T fovy, T aspect, T zmin, T zmax) {
  T A, B;
  TMat4<T> M;
...
template <class T> Mat4<T> lookat_matrix(const TVec3<T> &from, const TVec3<T> &at, const TVec3<T> &v_up) {
  ...
  TMat4<T> M(TVec4<T>(s, 0), TVec4<T>(u, 0), TVec4<T>(-f, 0),
             TVec4<T>(0, 0, 0, 1));

  return M * translation_matrix(-from);
}

template <class T> TMat4<T> viewport_matrix(T w, T h) {
  return scaling_matrix(TVec3<T>(w / 2.0, -h / 2.0, 1)) * translation_matrix(TVec3<T>(1, -1, 0));
}

template <class T> TMat4<T> operator*(const TMat4<T> &n, const TMat4<T> &m) {
  TMat4<T> A;
  int i, j;
...
template <class T> TMat4<T> adjoint(const TMat4<T> &m) {
  TMat4<T> A;
...
template <class T> T invert_cramer(TMat4<T> &inv, const TMat4<T> &m) {
  TMat4<T> A = adjoint(m);
  T d = A[0] * m[0];
...
template <class T> T invert(TMat4<T> &B, const TMat4<T> &m) {
  TMat4<T> A = m;
  int i, j, k;
  T max, t, det, pivot;
...
template <class T> inline void Trans2WT(const TMat4<T> &trans, TVec3<T> &w, TVec3<T> &t) {
  for (int y = 0; y < 3; ++y)
    t[y] = trans[y][3];
...
template <class T> inline void WT2Trans(const TVec3<T> &w, const TVec3<T> &t, TMat4<T> &trans) {
...
template <class T> inline TMat3<T> hat(const TVec3<T> &vec) {
