...
template <class T> inline bool TVec2<T>::operator==(const TVec2<T> &v) const {
  return m_elt[0] == v.m_elt[0] && m_elt[1] == v.m_elt[1];
};
...
template <class T> bool predVec20(const TVec2<T> &lhs, const TVec2<T> &rhs) {
  return lhs[0] < rhs[0];
};

template <class T> bool predVec21(const TVec2<T> &lhs, const TVec2<T> &rhs) {
  return lhs[1] < rhs[1];
};
...
template <class T> T cross(const TVec2<T> &lhs, const TVec2<T> &rhs) {
...
template <class T> T isIntersect(const TVec2<T> &p0, const TVec2<T> &p1, const TVec2<T> &q0, const TVec2<T> &q1) {
  ...
  const TVec2<T> q0p0 = q0 - p0;
  const TVec2<T> q0q1 = q0 - q1;
  const TVec2<T> q0p1 = q0 - p1;
  ...
  if (itmp0 == 0 && itmp1 == 0) {
    ...
    */
    ...
    const T v_p0 = 0;
    const T v_p1 = p0q0 * p0p1;
    const T v_q0 = p0q0 * p0q0;
  ...
  const TVec2<T> q1p0 = q1 - p0;
  const TVec2<T> q1p1 = q1 - p1;
