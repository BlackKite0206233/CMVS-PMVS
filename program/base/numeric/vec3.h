...
public:
  ...
  inline T norm2(void) const { return (*this) * (*this); }
  ...
  inline T sum(void) const { return m_elt[0] + m_elt[1] + m_elt[2]; }
};
...
template <class T> inline bool TVec3<T>::operator==(const TVec3<T> &v) const {
  return m_elt[0] == v.m_elt[0] && m_elt[1] == v.m_elt[1] && m_elt[2] == v.m_elt[2];
};
...
template <class T> inline std::ostream &operator<<(std::ostream &out, const TVec3<T> &v) {
  return out << v[0] << " " << v[1] << " " << v[2];
};

template <class T> inline std::istream &operator>>(std::istream &in, TVec3<T> &v) {
...
template <class T> inline void ortho(const TVec3<T> &z, TVec3<T> &x, TVec3<T> &y) {
  if (fabs(z[0]) > 0.5) {
    x[0] =  z[1];
    x[1] = -z[0];
    x[2] =  0;
...
template <class T> bool predVec30(const TVec3<T> &lhs, const TVec3<T> &rhs) {
  return lhs[0] < rhs[0];
};

template <class T> bool predVec31(const TVec3<T> &lhs, const TVec3<T> &rhs) {
  return lhs[1] < rhs[1];
};

template <class T> bool predVec32(const TVec3<T> &lhs, const TVec3<T> &rhs) {
  return lhs[2] < rhs[2];
};
...
template <class T> struct Svec3cmp {
