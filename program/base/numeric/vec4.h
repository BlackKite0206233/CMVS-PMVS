...
public:
  ...
  TVec4 &operator=(T s) {
    m_elt[0] = m_elt[1] = m_elt[2] = m_elt[3] = s;
    return *this;
  }
...
template <class T> inline bool TVec4<T>::operator==(const TVec4<T> &v) const {
  return m_elt[0] == v.m_elt[0] && m_elt[1] == v.m_elt[1] && m_elt[2] == v.m_elt[2] && m_elt[3] == v.m_elt[3];
};
...
template <class T> inline std::ostream &operator<<(std::ostream &out, const TVec4<T> &v) {
  return out << v[0] << " " << v[1] << " " << v[2] << " " << v[3];
};

template <class T> inline std::istream &operator>>(std::istream &in, TVec4<T> &v) {
...
template <class T> struct Svec4cmp {
...
template <class T>
inline void ortho(const TVec4<T> &z, TVec4<T> &x, TVec4<T> &y) {
  if (fabs(z[0]) > 0.5) {
    x[0] =  z[1];
    x[1] = -z[0];
    x[2] =  0;
