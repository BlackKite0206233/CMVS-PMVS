...
void CpatchOrganizerS::init(void) {
  m_pgrids.clear();
  m_pgrids.resize(m_fm.m_tnum);
  m_vpgrids.clear();
  m_vpgrids.resize(m_fm.m_tnum);
  m_dpgrids.clear();
  m_dpgrids.resize(m_fm.m_tnum);
  m_counts.clear();
  m_counts.resize(m_fm.m_tnum);

  m_gwidths.clear();
  m_gwidths.resize(m_fm.m_num);
  m_gheights.clear();
  m_gheights.resize(m_fm.m_num);
...
void CpatchOrganizerS::writePatches2(const std::string prefix, bool bExportPLY, bool bExportPatch, bool bExportPSet) {
  ...
  if (bExportPatch) {
    ...
    ofstream ofstr;
    ofstr.open(buffer);
    ofstr << "PATCHES" << endl << (int)m_ppatches.size() << endl;
  ...
  if (bExportPSet) {
    ...
    ofstr.open(buffer);
    for (int p = 0; p < (int)m_ppatches.size(); ++p)
      ofstr << m_ppatches[p]->m_coord[0]  << ' ' << m_ppatches[p]->m_coord[1]  << ' ' << m_ppatches[p]->m_coord[2]  << ' '
            << m_ppatches[p]->m_normal[0] << ' ' << m_ppatches[p]->m_normal[1] << ' ' << m_ppatches[p]->m_normal[2] << "\n";
    ofstr.close();
  }
}

void CpatchOrganizerS::readPatches(void) {
  // Read-in existing reconstructed points. set m_fix to one for non-targeting
  // images
    ...
    for (int p = 0; p < pnum; ++p) {
      ...
      for (int j = 0; j < (int)ppatch->m_vimages.size(); ++j)
        if (m_fm.m_tnum <= ppatch->m_vimages[j]) {
          cerr
              << "Impossible in readPatches. m_vimages must be targeting images"
              << endl
    ...
    for (int j = 0; j < 2; ++j) {
      for (int i = 0; i < 2; ++i) {
        ...
        if (m_dpgrids[image][index] == m_MAXDEPTH)
          ...
          const float dtmp = m_fm.m_pss.m_photos[image].m_oaxis * m_dpgrids[image][index]->m_coord;
...
void CpatchOrganizerS::setGrids(Ppatch &ppatch) const { setGrids(*ppatch); }
...
void CpatchOrganizerS::removePatch(const Ppatch &ppatch) {
  for (int i = 0; i < (int)ppatch->m_images.size(); ++i) {
    ...
    m_pgrids[image][index].erase(remove(m_pgrids[image][index].begin(),
                                        m_pgrids[image][index].end(), ppatch),
  ...
  for (int i = 0; i < (int)ppatch->m_vimages.size(); ++i) {
...
#ifdef DEBUG
    if (m_fm.m_tnum <= image) {
      cerr << "Impossible in removePatch. m_vimages must be targetting images"
           << endl;
      exit(1);
    }
#endif
    ...
    m_vpgrids[image][index].erase(remove(m_vpgrids[image][index].begin(),
                                         m_vpgrids[image][index].end(), ppatch),
  ...
  while (bimage != eimage) {
    ...
    for (int j = -margin; j <= margin; ++j) {
      ...
      for (int i = -margin; i <= margin; ++i) {
        ...
        vector<Ppatch>::const_iterator bpatch = m_fm.m_pos.m_pgrids[image][index].begin();
        vector<Ppatch>::const_iterator epatch = m_fm.m_pos.m_pgrids[image][index].end();
        while (bpatch != epatch) {
...
void CpatchOrganizerS::setScales(Patch::Cpatch &patch) const {
  ...
  for (int i = 1; i < inum; ++i) {
    Vec3f diff =
        m_fm.m_pss.project(patch.m_images[i], patch.m_coord,               m_fm.m_level) -
        m_fm.m_pss.project(patch.m_images[i], patch.m_coord - ray * unit2, m_fm.m_level);
    patch.m_dscale += norm(diff);
  }

  // set m_dscale to the vertical distance where average pixel move is half
  // pixel
...
void CpatchOrganizerS::writePLY(const std::vector<Ppatch> &patches, const std::string filename) {
  ...
  while (bpatch != bend) {
    ...
    ofstr << (*bpatch)->m_coord[0]  << ' ' << (*bpatch)->m_coord[1]  << ' ' << (*bpatch)->m_coord[2]  << ' ' 
          << (*bpatch)->m_normal[0] << ' ' << (*bpatch)->m_normal[1] << ' ' << (*bpatch)->m_normal[2] << ' '
